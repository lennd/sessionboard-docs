...
| Date | Result |
| --- | --- |
| 2026-08-07 | **Cutover complete — the Help Center is live at `learn.sessionboard.com`, the hostname the HubSpot KB used to serve.** HubSpot released the domain at 19:39 UTC (caught by a 5-minute poller), which dropped the Cloudflare-for-SaaS claim described in the entry below. Because the hostname was already bound to the Worker as a Custom Domain, no DNS change was needed: the first request after release was answered by our Worker and resolved a legacy KB slug correctly. The flip was one value in `site.json` (`canonicalHost` → `learn`, `legacyHosts` → `["help"]`), which every consumer reads — canonicals, `og:url`, JSON-LD, GA4 linker, `robots.txt` sitemap line, share-image footer labels, and the Worker's redirect direction all followed. Then `npm run og -- --force` (239 images), rebuild, deploy. Verified: `learn` serves 200 with self-canonicals, `help` 301s to `learn` in one hop, legacy KB slugs resolve in a single hop to the final `learn` path. **Audit: 311/311 resolve 301 → 200, 0 failures**, same 11 known fall-throughs. Sitemap re-submitted to GSC for the canonical host. Propagation of the reversed redirect took ~2 minutes. **The material win is that no ranking handover is needed** — the content now sits on the hostname holding all the history and backlinks, so the duplicate-content overlap noted below is closed. Also fixed en route: `audit_redirects.py` was flagging `/sitemap-0.xml` as a failure once `learn`'s sitemap started resolving through our Worker, since machine files serve 200 but have no article target — they are now excluded the same way HubSpot image assets are. |
| 2026-08-07 | **Launched at `help.sessionboard.com`; 311/311 legacy URLs verified against the live host.** The intended cutover was `learn.sessionboard.com` in place, and it is not currently possible: HubSpot holds that hostname as a **Cloudflare for SaaS custom hostname**, so Cloudflare returns **error 1034 (Edge IP Restricted)** for anything our zone tries to serve there — a proxied DNS record and a registered Worker route are both insufficient. Three approaches were tried and failed (proxied CNAME into HubSpot's zone → HubSpot kept answering via orange-to-orange; repointing to `192.0.2.1` → 1034; repointing to a real origin → 1034), costing **~6 minutes of 403s on `learn`** before rollback. `learn` is back on HubSpot and serving normally. So the site launched on `help.sessionboard.com` as a Worker **Custom Domain** (`AAAA → 100::`, no origin), which we control end to end, and `help` became the canonical host across `astro.config.mjs`, `Head.astro` canonicals/JSON-LD, the GA4 cross-domain linker, share-image labels, and `worker.js`. The Worker now 301s any `learn` request to the same path on `help` in a single hop, resolving legacy KB slugs on the way — so finishing the cutover is a DNS change with no code change, once a HubSpot admin releases the domain. Audit re-run against the live host: **311/311 resolve 301 → 200, 0 failures**, 11 fall-throughs (8 HubSpot `-temporary-slug-<uuid>` placeholders + 3 ambiguous drafts, all 404 in HubSpot today). Known cost until the domain is released: the same content is live on both hosts, so `help` will not rank meaningfully while `learn` holds the history. |
| 2026-08-07 | **Full 301 surface audit — 288/291 URLs verified.** The 220-article export was never the whole URL surface. Enumerated from four independent sources: live HubSpot sitemap (222 URLs), **Google Search Console** (270 URLs with search data over 16 months, via the `sc-domain:sessionboard.com` property), **Semrush** `backlinks_pages` (externally linked URLs), and a grep of `web-api` / `web-ui-v2` / `web-ui` for in-app links (23 unique). Union = **291 URLs**; **288 now 301 to a live page, 0 land on the generic fallback**. The 3 exceptions are HubSpot-hosted `/hs-fs/hubfs/` **images** (≈5 impressions in 16 months) — deliberately not redirected, since sending an image request to an HTML page is wrong. Gaps found and closed: (a) **9 KB category pages** (`frequently-asked-questions`, `training-videos`, `product-release-notes`, `portal-users`, `integrations`, `feature-overview-guides`, `sessionboard-how-tos`, `understanding-sessionboard-terms-roles`, `kb-search-results`) were live and ranking but absent from the article export entirely — 4 new section hub pages were written to give them real targets; (b) **`/en/migrated/knowledge-base/…`**, a live path prefix from an older HubSpot migration that Google still has indexed; (c) HubSpot's **own slug renames** (`6284057-create-assign-tasks` → `…-assign-tasks`) — now resolved by indexing on the numeric article ID, so future renames self-heal; (d) HubSpot serves **broken doubled-path 301s** (`/en/knowledge-base/en/knowledge-base/<slug>`) for two articles — normalized in the Worker; (e) **17 URLs** that previously fell through to the generic FAQ page now point at their nearest published article. Map grew 220 → 251 slugs. |
| 2026-08-07 | **Search-traffic baseline corrected.** `8103124-why-does-my-computer-say-this-site-can-t-be-reached` accounted for **4,905 of 8,256 clicks (59%)** and 787k of 1.25M impressions across 16 months — it ranked for the generic Chrome error string, not for anything about events. It was unpublished and `noindex`ed in HubSpot around April 2026 and has produced **0 clicks in the last 30 days**. The honest pre-cutover baseline is therefore **~262 clicks / 30 days** and **782 clicks / 90 days**, not the 16-month total. Recording this so the expected drop is not misread as migration damage. |
...
`redirects-301.csv` is the complete map: `old_url → new_path` for every article. `scripts/redirects-to-map.mjs` compiles it into `redirects-map.json`, which the Worker bundles; rows whose target is not in `dist/` are skipped, so a typo'd path can never become a live redirect.

The Worker resolves a legacy slug in four steps, falling through in order:

1. **Exact slug** match.
2. **Numeric prefix stripped** (`9156219-cvent-integration` ≡ `cvent-integration`).
3. **Numeric article ID** — HubSpot renames slug text while keeping the ID, so `6284057-create-assign-tasks` still resolves to whatever `6284057` is now. This is what makes the map survive HubSpot edits nobody told us about, and it also absorbs the punctuation variants Google indexed (`can-t` / `can't` / `cant`).
4. **Generic fallback** → the contact-support FAQ, so an unknown slug is never a 404.

It also normalizes two path quirks before matching: the `/en/migrated/knowledge-base/` prefix, and HubSpot's broken doubled `/en/knowledge-base/en/knowledge-base/` 301 targets. Query strings are ignored, which covers every `?hsLang=en` variant.

Draft and archived rows point at their intended new path where the article was eventually written, or at the nearest published article — see the audit log for the 17 that were repointed away from the generic fallback.
